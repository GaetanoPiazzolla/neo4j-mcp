...
description: >
  ...
  Translates questions into Cypher, executes them via MCPorter, and reasons over
  results before responding. Invoke for lookups, aggregations, graph traversals,
  rankings, stock checks, and relationship queries of any kind.
---

## Prerequisites

Requires `mcporter` installed globally (`npm install -g mcporter`) and a `.env` file
in the project root with `NEO4J_URI`, `NEO4J_USERNAME`, `NEO4J_PASSWORD`, `NEO4J_DATABASE`.

The `neo4j-mcp` binary must be on PATH. `read-cypher` validates queries server-side
via `EXPLAIN` before execution — no need to run EXPLAIN manually in this skill.
...
If the question involves labels or properties you are not sure about, call `get-schema`
first. Do not guess property names — wrong names produce empty results, not errors.
...
| Error | Action |
|---|---|
| `property not found` | Call `get-schema`, correct the property name, retry |
...
RETURN p.productName AS product, count(*) AS timesordered
ORDER BY timesordered DESC LIMIT 1
```
